...
export const UserItem = ({ id, label = 'Member', image, variant }: IUserItemProps) => {
		...
		<Button variant={'transparent'} className={cn(userItemVariants({ variant: variant }))} size={'sm'} asChild>
			<Link href={`/workspace/${workspaceId}/member/${id}`}>
				<Avatar className='size-5 rounded-md mr-1'>
					...
					<AvatarFallback className='rounded-md bg-red-800 text-white text-xs'>{avatarFallback}</AvatarFallback>
				</Avatar>
